...
async function wtl_init(wtlSwitchKey="wtlSwitch")
{
    ...
    {
        WTL_IS_ON = true;
        browser.tabs.onCreated.addListener(wtl_onCreatedHandler);
    }
    else if (requiredState == "off" && WTL_IS_ON)
    {
        WTL_IS_ON = false;
        browser.tabs.onCreated.removeListener(wtl_onCreatedHandler);
    }
}
...
{
    // in chromium based browsers, the new tabs open directly with the required link, HOWEVER, in Firefox, new tabs (leading to url) open as empty tab (about:blank) first
    // the differce between a user opened empty tab and a site that has been opened in a new tab, is the title "New Tab" being the one opened by the user, while a new tab with a site has the URL as the title
    let url;
    if (tab.url == "about:blank" && tab.title != "New Tab") url = "http://" + tab.title;
    else url = gh_getTabUrl(tab);
    if(wtl_skipThisTab(tab, url)) return;
    let newTabUrl = api_getExtFileHref("wait_to_load.html") + "#" + url;
    api_updateTab(tab.id, {url:newTabUrl});
}

// the url is a seperate parameter due to the Firefox issue
function wtl_skipThisTab(tab, url)
{
...
async function ts_init()
{
    ...
    if (requiredState == "on" && !TS_IS_ON)
    {
        ...
        browser.tabs.onCreated.addListener(ts_onCreatedHandler);
        browser.tabs.onActivated.addListener(ts_onActivatedHandler);
    }
    
    else if (requiredState == "off" && TS_IS_ON)
    {
        TS_IS_ON = false;
        browser.tabs.onCreated.removeListener(ts_onCreatedHandler);
        browser.tabs.onActivated.removeListener(ts_onActivatedHandler);
...
async function as_init()
{
    ...
    if (requiredState == "on" && !AS_IS_ON)
    {
        ...
        browser.tabs.onUpdated.addListener(as_onUpdatedHandler); // onUpdated also fires after a new tab is opened (url changes from blank to whatever)
    }
    else if (requiredState == "off" && AS_IS_ON)
    {
        AS_IS_ON = false;
        browser.tabs.onUpdated.removeListener(as_onUpdatedHandler);
    }
}
