...
var GLOBALS = browser.extension.getBackgroundPage(); // access global and background functions
...
async function qa_openLinks()
{
    ...
    let promises = [];
    for (let link of purifiedLinksArr) promises.push(openLink(link));
    await Promise.all(promises);
    await GLOBALS.wtl_init(); // return wait to load back to normal

    async function openLink(link)
    {
        await GLOBALS.api_openNewTab(link);
    }
}
...
async function tea_paste(textInputElem)
{
    textInputElem = oh_idToElem(textInputElem);
    let clipText = await navigator.clipboard.readText();
    textInputElem.value += clipText;
    textInputElem.dispatchEvent(new Event('change', {bubbles: true, cancelable: true})); // trigger the change event so it saves it to memory
}

function tea_replace(textInputElem, newText)
{
    textInputElem = oh_idToElem(textInputElem);
    textInputElem.value = newText;
    textInputElem.dispatchEvent(new Event('change', {bubbles: true, cancelable: true})); // trigger the change event so it saves it to memory
}
